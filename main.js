...
function parseList(element2, options) {
  return element2.children.flatMap((item) => {
    ...
    let parsedChildren = item.children.flatMap((child) => parseNode(child, options)).filter(child => child != null);
    if (parsedChildren == null) parsedChildren = [];
  ...
  updatePage(pageId, body) {
    return __async(this, null, function* () {
      ...
      const response = yield (0, import_obsidian4.requestUrl)({
        ...
        (error) => this.handleRequestError(error, `Updating Notion page ${pageId}`)
      );
      const data = yield response.json;
      this.debugLog("UploadBase", "Page update response received", {
    ...
    return __async(this, null, function* () {
      ...
      while (hasMore) {
        ...
        const response = yield (0, import_obsidian4.requestUrl)({
          ...
          (error) => this.handleRequestError(error, `Fetching children of page ${pageId}`)
        );
        if (response.status !== 200) {
          console.error(`Failed to fetch children of page ${pageId}:`, response.json);
          break;
        }
        ...
        for (const child of children) {
          yield (0, import_obsidian4.requestUrl)({
            url: `https://api.notion.com/v1/blocks/${child.id}`,
            method: "DELETE",
            headers: {
              Authorization: "Bearer " + notionAPI,
              "Notion-Version": NOTION_API_VERSION
            ...
            (error) => this.handleRequestError(error, `Deleting block ${child.id}`)
          );
        }
...
var Upload2Notion = class extends UploadBase {
  ...
  upsertGeneral(params) {
    return __async(this, null, function* () {
      ...
      if (params.notionId) {
        console.log(`[Upload2Notion] Updating existing Notion page`, {
          notionId: params.notionId
        });
        // 构建更新用的 body（不包含 children）
        ...
        // 移除 parent 和 children 字段，因为更新页面不需要
        delete body.parent;
        delete body.children;
        // 更新页面属性
        const updateResult = yield this.updatePage(params.notionId, body);
        // 清空现有 blocks
        yield this.clearPageBlocks(params.notionId);
        // 添加所有 blocks
        const allChunks = [firstChunk, ...extraChunks];
        if (allChunks.length > 0) {
          yield this.appendExtraBlocks(params.notionId, allChunks);
        }
        return updateResult;
      } else {
        const body = this.buildGeneralBody({
  ...
  upsertNext(params) {
    return __async(this, null, function* () {
      ...
      if (params.notionId) {
        console.log(`[Upload2Notion] Updating existing NotionNext page`, {
          notionId: params.notionId
        });
        // 构建更新用的 body
  ...
  upsertCustom(params) {
    return __async(this, null, function* () {
      ...
      if (params.notionId) {
        console.log(`[Upload2Notion] Updating existing custom page`, {
          notionId: params.notionId
        });
        // 构建更新用的 body
